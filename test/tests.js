...
describe('Parsing', () => {
  ...
  describe('Tuples', () => {
    it('Empty tuple', () => {
      parser.parse("tuple: ()").value.should.deep.equal({"Tuple":{}});
    });

    it('Basic tuple', () => {
      parser.parse("tuple: (123, False)").value.should.deep.equal({"Tuple":{"fst": 123, "snd":false}});
    });

    it('Arbitrary long tuple', () => {
      parser.parse('tuple: (123, "Some string.", True, 12.34)').value.should.deep.equal({"Tuple": {"fst": 123, "snd":"Some string.", "others": [true, 12.34]}});
    });

    it('Tuple in tuple', () => {
      parser.parse('tuple in tuple: (123, (True, 12.34))').value.should.deep.equal({"Tuple": {"fst": 123, "snd": {"Tuple": {"fst": true, "snd": 12.34}}}});
    });
  });
  ...
  describe('List', () => {
    ...
    it('List of tuples', () => {
      parser.parse("list: [(\"s1\",\"s2\"),(\"s3\",\"s4\")]").value.should.deep.equal([{"Tuple":{"fst":"s1", "snd": "s2"}}, {"Tuple":{"fst": "s3", "snd":"s4"}}]);
    });
  });

  describe('Dict', () => {
    it('Empty dict', () => {
      parser.parse("dict: Dict.fromList []").value.should.deep.equal({"Dict": []});
    });
    it('Filled dict', () => {
      parser.parse("dict: Dict.fromList [(1,\"a\"),(2,\"b\")]").value.should.deep.equal({"Dict": [{"Tuple":{"fst": 1,"snd":"a"}},{"Tuple":{"fst":2,"snd":"b"}}]});
    });
  });
  ...
  describe('Custom types', () => {
    it('Custom type with one value', () => {
      parser.parse("custom type: User \"Adam\"").value.should.deep.equal({"User": "Adam"});
    });
    it('Custom type with more values', () => {
      parser.parse("custom type: User \"Adam\" 123 (1,False)").value.should.deep.equal({"User": ["Adam", 123, {"Tuple": {"fst": 1, "snd": false}}]});
    });
    it('Custom type in parenthesis', () => {
      parser.parse("custom type: (User (Data \"Adam\" 123 (1,False)))").value.should.deep.equal({"User": {"Data": ["Adam", 123, {"Tuple": {"fst": 1, "snd": false}}]}});
    });
    it('Custom type in parenthesis with record', () => {
      parser.parse("custom type: (User { age = 23 })").value.should.deep.equal({"User": {"age": 23}});
    });
