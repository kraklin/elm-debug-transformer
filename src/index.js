  ...
  window.devtoolsFormatters = [{
    header: function(obj, config){
        if ((!!obj.type && obj.type === 'ElmDebug')||(!!config && config.elmFormat)){
          const prefix = (!!config && !!config.key) ? [["span", {style: "color: purple; font-weight: bold"}, config.key], ["span", {}, " = "]] : [];

          return ["div", {}, "(h)"].concat(prefix).concat(handleHeader(obj,config))
        }
        else { return null; }
    },
  ...
  switch(value.type){
    ...
    case 'Record':
      const keys = _.chain(value.value)
        .mapValues((v, k)=> {return k+" = "+ handleHeader(v, config)})
        .values()
        .value();
  ...
  switch(value.type){
    ...
    case 'Record':
      const values = _.chain(value.value)
        .mapValues((v, k)=> {if(_.isString(v)) { return ["span",{style: "color:red"},k+"=" + v] } return ["div", {}, ["object", {object: v, config: {key: k, elmFormat: true}}]]})
        .values()
        .value();
